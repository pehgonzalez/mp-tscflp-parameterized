...
(T_{\mathrm{orac}}+n|L|)\bigr)$ for $k\ge 1$, and in time
$O(T_{\mathrm{orac}}+n|L|)$
...
only the empty design and is decided by one oracle call. Each support
...
giving $O\bigl(n^{k}(T_{\mathrm{orac}}+n|L|)\bigr)$ in total. For
...
within $O(T_{\mathrm{orac}}+n|L|)$. Comparing the minimum
...
\item \kw{if} $\mathrm{best}<+\infty$ \kw{and} $\mathrm{best}\le B$
  ...
  either way with $\OPT_k=\mathrm{best}$
...
holds at most
$\sum_{m=0}^{n}\sum_{t=0}^{k+1}\binom{m}{t}=O(n^{k+2})$ nodes. Each
node performs the sorts and prefix scans of P1 and at most one oracle
call, so the whole search runs in
$O\bigl(n^{k+2}(T_{\mathrm{orac}}+n|L|\log n)\bigr)$ time, a
...
\begin{observation}[the covering prune in plain optimisation]
...
completion within the remaining budget $r$ exists, which P1 detects
