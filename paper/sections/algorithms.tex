...
in the number of opened facilities together with the branch-and-bound
...
$e\,n^{k}$ candidate supports, and each is priced by one oracle call.
...
$k$, and the branch-and-bound that follows keeps the same worst case
while pruning aggressively in practice.
...
normalise $k\le n$. Then $\mathrm{MP\text{-}TSCFLP}(B,k)$ is decidable,
and the least cost $\OPT_k$ over feasible designs opening at most $k$
facilities is computable, in time $O\bigl(n^{k}\cdot
T_{\mathrm{orac}}\bigr)$ for $k\ge 1$, and in time $O(T_{\mathrm{orac}})$
for $k=0$. In particular the problem lies in \XP\ in the parameter $k$.
\end{theorem}
\begin{proof}
...
routing value is attained, so each feasible support realises exactly its
...
$\OPT_k=+\infty$, and the correct answer is then \textsc{no}. Enumerate
every such $S$, test feasibility
by Proposition~\ref{prop:feas} in $O(n|L|)$ time, and price the feasible
...
nonnegative terms. The case $n=0$ needs no counting, since it leaves
only the empty design and is decided by one oracle call. Each support costs $O(T_{\mathrm{orac}})$, and
one oracle call already reads the $n|L|$ capacities, so
$T_{\mathrm{orac}}=\Omega(n|L|)$ and the oracle time dominates the
feasibility test, giving $O(n^{k}\,T_{\mathrm{orac}})$ in total. For
...
within $O(T_{\mathrm{orac}})$. Comparing the minimum
against $B$ decides $\mathrm{MP\text{-}TSCFLP}(B,k)$.
\end{proof}

The enumeration is wasteful whenever a partial design already cannot be
completed within the budget, and the branch-and-bound of
Algorithm~\ref{alg:xpbb} closes such subtrees early. Its principal device
...
\end{lemma}
\begin{proof}
Both directions follow from majorisation by the sorted prefix. If
...
quantities of line 2 are the root specialisations
...
\label{alg:xpbb}
\begin{pseudolines}
\item $k\gets\min(k,n)$ \algcomment{normalisation, no design opens more}
...
\item \kw{if} $\mathrm{best}\le B$ \kw{then} \kw{return} \textsc{yes} with
  $\OPT_k=\mathrm{best}$ \kw{else} \kw{return} \textsc{no}
  \algcomment{run with $B=\infty$ to return $\OPT_k$ unconditionally}
\end{pseudolines}
\end{algorithm}
...
\end{proposition}
The full argument appears in \ref{app:tech}. Its three steps justify
...
None of the prunes changes the worst case. On the seeded validation suite
...
\input{figures/fig_dichotomy.tex}

\begin{theorem}[fixed-parameter tractability]\label{thm:fpt}
$\mathrm{MP\text{-}TSCFLP}(B)$, its optimisation form and its cardinality
...
yields an inequality valid across the whole hypercube of designs.
Figure~\ref{fig:bendersflow} traces the pipeline the proposition
formalises.

\input{figures/fig_benders_flow.tex}
...
\end{proposition}
The proof, given in \ref{app:tech}, writes the block dual explicitly.
...
Customers with identical cost columns aggregate exactly, products do not, a partial bound on the data admits a polynomial
compression through the weight-reduction technique of
\citet{FrankTardos1987}, and yet no polynomial kernel exists in
...
polynomial time.
\end{proposition}
The proof, given in \ref{app:tech}, exhibits the two maps directly. The
merge adds the flow rows of the two customers, the split expands the
merged row by the northwest-corner procedure, and both preserve every
constraint and every cost term, so the two instances agree design by
design.

A companion reduction bounds the capacities without touching any design.
...
\end{observation}
The proof, given in \ref{app:tech}, trims any routing until every
...
aggregation. A partial bound on the data still buys
a genuine compression, along the route that
Figure~\ref{fig:ftpipeline} lays out.

\input{figures/fig_ft_pipeline.tex}

\begin{proposition}[one-sided compression]\label{prop:ftcompress}
...
\end{proposition}
The proof, given in \ref{app:tech}, first aggregates customers and caps
capacities, so that acceptance becomes a Boolean combination of sign
conditions on integer linear forms in the cost vector, with coefficients
that total unimodularity bounds by a fixed power of $\kappa$. The
Frank--Tardos theorem then replaces the vector by one of
$\mathrm{poly}(\kappa)$ bits preserving every sign at once, and the
second subclass exchanges the roles of the two data groups.
...
Each instance falling in a reserved class is decided individually in
polynomial time by the stated test, an empty universe being covered by
the empty subfamily, a target of zero over a nonempty universe admitting
no cover, and a target above $m$ admitting one exactly when the whole
family covers the universe. On inputs from those classes the composition
therefore answers the OR of those individual answers, emitting a fixed
yes- or no-instance realising it. Deciding $R$ compares three numbers, and
...
other depot at cost $1$, so an unguarded pair costs at least $Q$ and at
...
The incidence structure comes first.
...
$\mathrm{MSO}_1$ optimisation logic over the unweighted incidence graph
...
as objects are not addressed by this observation and remain unanalysed
