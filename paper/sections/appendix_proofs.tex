...
grouped by result, one subsection each, so that a remission from the
...
$t=s-a$ linearises the inner minimum, since
\[
...
amortised time, every index entering and leaving the queue once per
...
polynomial in $\nu$ and $\log N_0$, an integer vector $\Psi'$ of
