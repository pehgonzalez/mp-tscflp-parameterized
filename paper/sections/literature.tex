...
and effective genetic algorithm for the single-product two-stage capacitated problem
...
extending a single-product hybrid from the same group. Across this line the problem is treated as empirically hard, and
...
distinction between weak and strong \NP-hardness that organizes our
numeric layer is that of \citet{GareyJohnson1979}.

Parameterized complexity has been brought to bear on optimization and
...
transfer to the incidence representation analysed here. Richer
representations of an instance might behave differently, and we leave
them unanalysed rather than excluded. The contrast is sharpest on the
approximation side, where what separates our covering core from the
capacitated clustering results is the absence of a metric. Without a
metric the parameterized lower bounds have
...
fixed-parameter tractable problem $\mathrm{W}[2]$-hard. Zero fixed charges play a comparable role here, for with every
charge at zero the problem turns polynomial, and our constructions
need large numbers, many products or a positive charge somewhere to
produce hardness.
...
analysis of the two-stage capacitated multiproduct problem, and building
that analysis requires first pinning the model down, which the next
section does.
...
\checkmark & \\
\citet{FellowsFernau2011} & & & & \checkmark & & \checkmark \\
