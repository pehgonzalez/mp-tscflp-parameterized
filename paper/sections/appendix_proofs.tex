...
paragraph naming the route of its proof, and the proofs appear here in
statement order.
...
$\sum_i a_i=\sum_j e_j$ are realised by some plan, for instance the
...
capacities, in which case the instance is infeasible. The table is
computed
in $O(|I|\cdot D\cdot b_{\max})$ time, since each of the $|I|\cdot(D+1)$
states scans at most $b_{\max}+1$ values of $a$. The depot problem is the
same recurrence with $g_j$, $\delta_j+d_j$ and $p_j$, in $O(|J|\cdot
D\cdot p_{\max})$ time, its value $+\infty$ likewise signalling
...
\emph{Conclusion.} If $\OPT_k\le B$ then $\mathrm{best}=\OPT_k\le B$ and
the algorithm returns \textsc{yes}. If $\OPT_k>B$ then
$\mathrm{best}\ge\OPT_k>B$ by soundness and it returns \textsc{no}, and
the answer \textsc{no} of line 3, when that return fires instead, is
correct by the early-return argument. Either
...
retains its initial value $+\infty=\OPT_k$, and when the early return
fires instead line 3 reports the same infinite optimum.
\end{proof}
...
$\mu_l(y,z)$ in time $O(|I||J|+|J||K|)$. Moreover, writing $\theta_l$ for
...
cuts and the rays. Write the block primal as the minimisation
of
...
feasibility nor the optimal value of the block, and we dualise it in
...
rays realise the feasibility cuts. The ray $\alpha\equiv 1$, $\beta\equiv
...
answers of both decision versions, while complete solutions correspond
under the explicit merge and split maps of the proof. Consequently one
...
optimal designs and the same decision answers, and the merge and split
maps just exhibited carry the complete solutions across. Iterating over
...
\medskip\noindent
\textbf{Proposition~\ref{prop:ftcompress} (one-sided compression).}
...
most $\kappa^{c^\star}$, with capacities unrestricted, admits a
polynomial compression in $\kappa$ into a fixed sign-condition
language, which is not itself a location problem, and so does the
subclass with $\max(f,g,c,d,B)\le\kappa^{c^\star}$. The first subclass is nontrivial.
...
$\max_l D_l\le\kappa^{c^\star}$. After these two reductions the
entities number $\mathrm{poly}(\kappa)$, every capacity and demand
entry is an integer at most $\kappa^{c^\star}$, and within the cost
vector $\Phi=(f,g,c,d,B)$ only the budget escapes the bound. The
compression treats the whole of $\Phi$ at once.

For any fixed design $S$ the routing value is the sum over products $l$
of a minimum-cost flow in the
network $N_l(S)$ of Proposition~\ref{prop:oracle}, each per-product
minimum is attained at a basic (vertex) flow, and a basic flow of the
design means one basic flow per product, each of value $D_l$. The
...
lies in $\{-1,0,1\}$ as well. Each basic-flow value is therefore a
$\{-1,0,1\}$-combination of the right-hand sides of its network. After
the two reductions that network carries at most a fixed multiple of
$\kappa^{2}$ rows and arc bounds, every right-hand side is at most
$\max_l D_l\le\kappa^{c^\star}$, and every basic-flow value is an
integer of magnitude at most a fixed multiple of $\kappa^{c^\star+2}$.

The instance is a yes-instance if and only if some pair of a design and a
basic flow of its network has total cost at most $B$. Each such test
reads $a\cdot\Phi\le 0$, where the coefficient vector $a$ collects the
design bits on the entries $f,g$, the basic-flow
values on the entries $c,d$, and the entry $-1$ on the budget $B$. These
coefficients are integers of magnitude at most a bound
$\Delta_{\max}$ that is a fixed multiple of $\kappa^{c^\star+2}$, and
they are fixed independently of $\Phi$ once the design and basis are
fixed, so acceptance is a Boolean combination of sign conditions on
integer linear forms in $\Phi$ of dimension $\nu=\mathrm{poly}(\kappa)$
with coefficients bounded by $\Delta_{\max}$. The Frank--Tardos theorem
\citep{FrankTardos1987,EtscheidKratschMnichRoeglin2017} produces, for
any prescribed bound $N_0$, in time
polynomial in $\nu$ and $\log N_0$, an integer vector $\Phi'$ of
$\mathrm{poly}(\nu,\log N_0)$ bits such that
$\operatorname{sign}(a\cdot\Phi')=\operatorname{sign}(a\cdot\Phi)$ for
every integer vector $a$ whose $\ell_1$-norm is strictly below $N_0$.
We invoke it with $N_0=\nu\,\Delta_{\max}+1$. Every acceptance form has
$\lVert a\rVert_{\infty}\le\Delta_{\max}$ and hence
$\lVert a\rVert_{1}\le\nu\,\Delta_{\max}=N_0-1$, so every form is
strictly covered, and $\log N_0=O(\log\kappa)$ for fixed $c^\star$, so
$\Phi'$ has $\mathrm{poly}(\kappa)$ bits. Replacing $\Phi$ by $\Phi'$
preserves the sign of every acceptance form at once and thus the
answer. The output couples the reduced numeric data with $\Phi'$, and
it is a compression onto the fixed language of these sign conditions
rather than a well-formed instance of $\mathrm{MP\text{-}TSCFLP}(B)$,
because entries of $\Phi'$ may be negative, while its encoding length
is $\mathrm{poly}(\kappa)$. For each fixed $c^\star$ that target
language is a fixed problem, and the compression framework places no
requirement that it lie in \NP.

In the second subclass the costs and the budget are at most
$\kappa^{c^\star}$ and the large numbers are the capacities and
demands, so the theorem applies instead to the extended vector
$\Psi=(b,p,q,1)$, whose final coordinate absorbs the additive constants
below. Customer aggregation again brings the dimension of $\Psi$ down
to $\mathrm{poly}(\kappa)$. For a fixed design and a fixed basis of each
product's network, every basic-flow value is again a
...
contribute further forms with coefficients in $\{-1,0,1\}$. Invoked as
above, the theorem compresses $\Psi$ to $\mathrm{poly}(\kappa)$ bits
while preserving every sign at once. The output pairs the unchanged
small data with the compressed vector, the Boolean combination of sign
conditions still decides acceptance, and the map is again a polynomial
compression onto such a sign-condition language. With
