...
hundred-instance benchmark and proposed two hybrid metaheuristics for the
problem, a clustering search matheuristic and a biased random-key genetic
algorithm with local search. \citet{MoraisEtAl2022} later combined a
biased random-key genetic algorithm with local branching for the same
multiproduct problem, extending a single-product hybrid from the same
group.
...
mixed-integer solver closed none to proven optimality within a one-hour
...
a feature whose cost no exact method avoids. The
...
first-stage cost delimits a pseudo-polynomially solvable part, and an
explicit non-separable family is strongly hard, the general boundary
remaining open (Theorem~\ref{thm:numeric} and
...
Each gadget was validated by a battery of on the order of nine hundred
thousand individual checks with no failure, and the enumeration algorithm agrees
with an independent brute-force solver across all $1{,}381$
comparisons. The computational study asks in what regime the
enumeration is practical, how it stands against Gurobi, the
mixed-integer baseline of Section~\ref{sec:experiments}, and whether
the pruning threshold $k^{\ast}$ that our branch-and-bound derives,
computed by one sort and one prefix scan before any optimisation,
tracks the empirical difficulty of an instance, and through which
mechanism. The protocol, instances and seeds are fixed in advance so
