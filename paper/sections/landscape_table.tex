The results of Sections~\ref{sec:prelim} to~\ref{sec:algorithms}
...
through the covering layer, the size-parameter dichotomy, the parameter
...
Sections~\ref{sec:prelim} to~\ref{sec:algorithms}.}
\label{tab:landscape}\\
\toprule
...
\midrule
\multicolumn{4}{l}{\emph{Covering layer}}\\
...
Cor.~\ref{cor:coverpara}, Cor.~\ref{cor:parak}, Cor.~\ref{cor:unary}\\
...
$k$, algorithm & both & \XP, $O(n^{k}(T_{\mathrm{orac}}+n|L|)+|K||L|)$ &
Thm.~\ref{thm:xp}\\
$k$, fine bound & both & no $f(k)N^{o(k)}$ under ETH &
Thm.~\ref{thm:eth}\\
a priori bound $k^\ast$ & both & valid lower bound on the opening
count, exact per side and product & Lem.~\ref{lem:cover}\\
\midrule
...
kernel in $\kappa=|I|+|J|+|L|+\tau$, costs and total demands bounded
by $\kappa^{c^\star}$ (capacities and $B$ free) & binary &
polynomial kernel & Prop.~\ref{prop:ftcompress}\\
compression in $\kappa$, costs and $B$ bounded by $\kappa^{c^\star}$
(capacities and demands free) & binary &
