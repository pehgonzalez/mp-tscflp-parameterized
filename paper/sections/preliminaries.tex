...
three longest proofs collected in \ref{app:proofs}. Each is additionally checked
...
$j$ can reach customer $k$ cheaply and to $1$ otherwise. Choosing which
...
A core of the model's combinatorial difficulty is thus already present
in the second stage, and the reductions of Section~\ref{sec:complexity}
grow from this observation.

The optimisation problem has two decision versions. In the first only the
...
\input{figures/fig_oracle_network.tex}
...
test (F1) and (F2) is exact. The full proof is in \ref{app:proofs}.

The characterisation rests on the completeness of the two stages in an
...
longer determine feasibility, which is why the characterisation is
...
sections use the proposition. The full proof is in \ref{app:proofs}.
...
routing is forced and the optimum admits a closed form.
...
\end{proposition}
\begin{proof}
Feasibility forces the design, and the design in turn forces the
routing. By Proposition~\ref{prop:feas}, a design $(y_1,z_1)$
...
$n^{k+O(1)}\cdot\mathrm{poly}(|I|,|J|,|K|,|L|)$. When $n=0$ the empty
...
assume $n\ge 1$. One may further normalise $k\le n$, since no design
