...
already forces strong NP-hardness and logarithmic inapproximability.
The same covering mechanism then reappears with the products in place
of the customers as the covered elements, and finally with the
factories themselves in that role, once tight capacities turn them
into carriers of demand. The
...
solution. Three recurring terms organize the section. The covering
...
same mechanism to the products, discriminated by the factories in the
first and by the depots in the second, and
...
Table~\ref{tab:reductions} summarizes the constructions side by side.

\input{tables/tab_reductions.tex}

\subsection{Two restrictions already suffice}\label{sec:cover}

After the preliminaries the first question is how much of the problem can
...
\input{figures/fig_coverage.tex}
...
algorithm approximates the optimization version of MP-TSCFLP within a
...
\citet{DinurSteurer2014} concerns exactly that minimization problem,
...
$\mathrm{MP\text{-}TSCFLP}(B)$ is strongly para-NP-hard with respect
to $|L|$, with respect to $|I|$, and with respect to the pair
$(|L|,|I|)$. Unless
...
slice of each parameter is already NP-hard, and strongly so by the
same theorem. A parameterized problem
...
Before the covering elements migrate to other dimensions of the problem,
...
subclass the problem contains.
\end{remark}

\subsection{Relocating the covering elements}\label{sec:products}
...
the discriminating layer from the depots to the factories, and the
hard group moves to $|J|=|K|=1$ accordingly.
...
The stages of the problem are not interchangeable, because the balance
...
the knapsack-cover of minimizing a subset sum subject to reaching $D$.
...
Theorem~\ref{thm:partition} yields the reduction, and only the
following changes are needed. The items become depots with $g_j=p_j=a_j$, the single
...
$B=A/2$ are unchanged. The two feasibility tests exchange
roles in the correctness argument, (F1) reading $b_1y_1=(A/2)y_1\ge A/2$ with $A>0$ by
...
Together the two weak slices delimit the quadrant of the problem in which
...
requires $p_1\ge\sum_k q_k$, a test supplied by (F2) whose
...
covering layer needs a facility layer that discriminates a dimension
carrying demand, the factories under tight capacities included, and
here none does, so the weak numeric hardness alone remains, vanishing
under a unary encoding.
\end{observation}
...
unit depot charges, so on this slice the budget simply counts the open
depots. No such reading exists in general, for a budget has no scale
of its own and rescaling all costs rescales it freely, which is why we
state the budget result only on this slice.
...
Observation~\ref{obs:xp} achieves exactly such a bound. What the
theorem excludes is every running time whose exponent grows sublinearly
in the parameter. For every computable $f$ and
every function $h$ with $h(t)/t\to 0$ no algorithm decides the target in
time $f(t)\,N^{h(t)}$, and this class is preserved by the composition,
which maps an algorithm with
...
test compares binary sums and, worse, minimizes an internal routing flow
...
The covering reductions accounted for three discrimination pairs, each
pairing a facility layer with a demand dimension, depots against
...
factory ships exactly $Q$. Since $\sum_j (X_j-w_j)=0$ with every
$X_j-w_j\ge 0$ by~\eqref{eq:C2}, $X_j=w_j$ for all $j$. A closed depot
...
Theorem~\ref{thm:numeric} shows that strong
hardness persists outside the separable class. The classification of the territory between the two,
including structured non-separable matrices such as low-rank or Monge
matrices, remains open.

\begin{theorem}[separable first stage]\label{thm:separable}
...
The proof, given in Appendix~\ref{app:sep}, reduces the routing cost to a
function of the two load marginals, realizes any consistent pair of
...
Corollary~\ref{cor:partitionmirror}, Observation~\ref{obs:quadrant} and
Theorem~\ref{thm:separable},
...
Theorem~\ref{thm:numeric}, and Corollary~\ref{cor:coverpara} again
supplies the parameterized reading, now robust under a unary encoding because
the instance numbers of Theorem~\ref{thm:numeric} are polynomial. For
the remaining claims of the statement, the four positive results match
their slices as follows. The dynamic program of
Theorem~\ref{thm:partition} covers the group $|J|=|L|=|K|=1$, its
layer-exchanged form in Corollary~\ref{cor:partitionmirror} covers the
mirror group $|I|=|L|=|K|=1$, the
...
part of the group $|K|=|L|=1$. Each of the four runs in
