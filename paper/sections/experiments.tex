...
a failure of protocol. At $n\ge 40$ the search processes a median of roughly
$2{,}500$ nodes before the limit, while a completed search at $n=24$
already needs a median of roughly $23{,}000$, and the cost of each node
...
partially solved cells are biased downward by that conditioning. Under
...
The stored solver logs admit a mediation-style analysis that points
to the mechanism, extracted by a pinned script with no further runs. The
...
association of the bound with solver effort is therefore consistent
with mediation through the relaxation. Tight capacities give tight root relaxations, and
