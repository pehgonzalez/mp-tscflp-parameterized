...
serve any customer. The uncapacitated single-product ancestor is
the classical facility location problem, and the two-stage
...
branching to a genetic-algorithm hybrid for the same problem.
...
multiproduct problem, and we give what is, to our knowledge, the first. We
...
What it leaves for practice is a lower bound on the number of facilities
any feasible design must open, computable from the capacities alone
before any optimization is run.
...
hard groups, each fixing two of the four dimensions to one. A single
mechanism, which we call a discrimination pair, drives all four. One
facility side faces another dimension of the instance, depots facing
customers or factories facing products for example, and its cost
matrix assigns each member of that dimension a distinct zero-one
pattern, which is enough to encode a covering problem.
Section~\ref{sec:complexity} fixes the notion precisely, and since
some pair survives every single restriction, so does the hardness.
...
trivialized, governs the smallest instances. With a single customer and
...
\NP${}\subseteq{}$coNP/poly (Theorem~\ref{thm:nokernel}), although a
polynomial kernel does appear once costs and total demands are
polynomially bounded in the larger parameter
$|I|+|J|+|L|+\tau$, with $\tau$ the number of distinct second-stage
cost columns (Proposition~\ref{prop:ftcompress}). Graph width explains none of this difficulty, for the incidence
graph of an instance always has clique-width at most two
(Observation~\ref{obs:cliquewidth}), so over that representation the
obstruction is arithmetic, living in the capacities and demands rather
than in the shape of the network. Several further positive
...
how the pruning threshold $k^{\ast}$ that our branch-and-bound
derives, available before any optimization is run, relates to the
gaps a mixed-integer solver reaches under a fixed limit, and at which
stage of the solving process the association is anchored. The protocol, instances and seeds are fixed in advance so
that every figure can be regenerated.

The remainder of this paper is organized as follows.
Section~\ref{sec:literature} places the problem in the exact, heuristic and
