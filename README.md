...
4. Statistical analysis. `python3 scripts/analyze_q3_correlation.py`
   ...
   seeds 20260718 and 20260719, the split inputs `controlled_mip.csv`,
   `controlled_bd.csv` and `mauri_mip.csv` are rebuilt from the combined
   runner output by `python3 scripts/split_mip_results.py`, and
...
5. Campaigns. `code/run_q1_rerun_par.sh` is the resumable Linux runner
   ...
   records shipping in `results/q1_rerun_lines/` and being joined into
   `results/q1_xp.csv` by
